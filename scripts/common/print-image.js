...
function distance(c1, c2) {
  const [h1, s1, l1] = cc.hex.hsl('#' + c1.toString(16));
  const [h2, s2, l2] = cc.hex.hsl('#' + c2.toString(16));
  return Math.sqrt(Math.pow(h2 - h1, 2) + Math.pow(l2 - l1, 2));
}

function closestColor(color) {
  color = parseInt(color.toString(16).replace(/ff$/, ''), 16);
  let result = 0;
  let dst = distance(result, color);
  for (const p of palette) {
    const pDst = distance(p, color);
    if (pDst < dst) {
      dst = pDst;
      result = color;
    }
