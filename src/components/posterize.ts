...
export function colorDistance(c1: string, c2: string) {
...
export function posterize(colors: string[][], palleteSize: number) {
  ...
  return colors.map((row) => row.map((cell) => colorsMap[cell]));
}
